...
const useBodyBlock = ({
  ...
  const handleScroll = (e: WheelEvent | TouchEvent) => {
    if (
      !Boolean((e.target as HTMLDivElement).closest('.' + scrollableItemClass)) ||
      scrollableItemParentRef.current!.clientHeight ===
        scrollableItemParentRef.current!.scrollHeight
    ) {
      e.preventDefault();
    }
  };
