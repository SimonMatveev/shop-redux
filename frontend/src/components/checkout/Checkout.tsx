                      ...
                      <ul className='checkout__cart-platforms'>
                        {item.orders.map((order, index) => (
                          <li className='checkout__cart-platforms-item' key={index}>
                            ...
                            <div className='checkout__cart-platforms-amount'>{order.amount}&nbsp;шт.</div>
                          </li>
                        ))}
                      </ul>
            ...
            <div className='checkout__pay'>
              <div className='checkout__sum-container'>
                <p className='checkout__sum-header'>
                  Итого: {totalAmount} товар{totalAmount === 1 ? '' : (totalAmount % 10 < 2 || totalAmount % 10 > 4) ? 'ов' : 'а'}
                </p>
                <p className='checkout__sum'>
                  {cart.totalPrice} руб.
                </p>
                {isWithSale && <p className='checkout__sumWithSale'>Выгода: <span>{cart.totalPrice - cart.totalPriceWithSale} руб.</span></p>}

              </div>
              <div className='checkout__pay-bottom'>
                <p className='checkout__result'>К оплате: <span>{cart.totalPriceWithSale} руб.</span></p>
                <button type='button' className='checkout__btn' onClick={() => setIsModalOpen(true)}>Перейти к оплате</button>
              </div>
            </div>
