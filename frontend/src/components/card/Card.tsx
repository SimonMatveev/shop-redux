...
const Card: FC<ICardProps> = ({ item }) => {
  ...
  let inCartIndex = useMemo(() => {
    return data
      ? data.cart.items.findIndex((cartItem) => cartItem.itemInCart._id === item._id)
      : -1;
  }, [data]);
  ...
  const getPlatformNameFromData = () =>
    PLATFORMS.find(
      (platform) => platform.id === data!.cart.items[inCartIndex]?.orders[0].platform
    )?.name || '';
  ...
  return (
    <article className='card'>
      <Link className='card__wrapper' to={`/items/${item._id}`}>
        ...
        {item.rating !== -1 && (
          <div className={`card__rating card__rating_color_${ratingColor}`}>
          ...
          <div className='card__prices'>
            {!isOnSale ? (
              ...
              <>
                ...
                <p className='card__price card__price_withsale'>
                  {item.priceWithSale} руб.
                </p>
              </>
            )}
          </div>
        ...
        {!isLoading && inCartIndex !== -1 ? (
          <>
            ...
            <span className='card__platform-first'>{getPlatformNameFromData()}</span>
          </>
        ) : (
          <button
            type='button'
