...
const ItemPage: FC = () => {
  ...
  const getIdFromSeries = (series: string) =>
    seriesList?.find((item) => item.name === series)?.id || 0;
  let inCartItem = useMemo(
    () =>
      currentUser
        ? currentUser.cart.items.find((item) => item.itemInCart._id === itemId) || null
        : null,
    [currentUser]
  );
    ...
    <section className='item-page'>
      <Container newClass={item ? 'item-page__container' : ''}>
        {!isLoading ? (
          item ? (
            <>
              <Link className='item-page__back' to='/items'>
                &lt; На главную
              </Link>
              ...
              <ul className='item-page__categories'>
                {item.category.map((category, index) => (
                  ...
                  >
                    ...
                    >
                      {getNameFromId<ENUM_CATEGORY>(CATEGORIES, category)}
                    </Link>
                  </li>
                ))}
              </ul>
              ...
              <div className='item-page__content'>
                <div
                  className={`item-page__instock${item.inStockAmount > 0 ? ' item-page__instock_active' : ''}`}
                >
                  {item.inStockAmount > 0 ? 'В наличии' : 'Товар закончился'}
                </div>
                <ul className='item-page__platforms'>
                  ...
                  {item.platforms.map((platform, index) => (
                    <li
                      key={index}
                      className='item-page__platform'
                      onClick={() => resetFilters()}
                    >
                      <Link
                        to={`/items?platforms=${platform}`}
                        className='item-page__platform-link'
                      >
                ...
                <div className='item-page__btns'>
                    ...
                    <ul className='item-page__orders'>
                      {inCartItem.orders.map((order, index) => (
                        <li key={index} className='item-page__order'>
                          <p className='item-page__order-platform'>
                  ...
                  <div className='item-page__add-block'>
                    {((inCartItem && inCartItem.orders.length < item.platforms.length) ||
                      !inCartItem) && (
                      <button
                        className={`item-page__btn${upIsLoading ? ' item-page__btn_loading' : ''}`}
                      ...
                      >
                        {upIsLoading
                          ? 'Загрузка...'
                          : inCartItem
                            ? 'Приобрести для других платформ'
                            : 'Добавить в корзину'}
                      </button>
                    )}
                    {promptIsOpen && (
                      <PromptForPlatforms
                        setUpIsLoading={setUpIsLoading}
                        newClass='item-page__prompt'
                        item={item}
                ...
                <p className='item-page__studio'>
                  Студия: <span>{item.studio}</span>
                </p>
                <p className='item-page__release'>
                  Дата выхода: <span>{item.releaseDate}</span>
                </p>

                <p className='item-page__description'>{item.description}</p>
                {item.series && (
                  <p className='item-page__series'>
                    Эта игра входит в серию:{' '}
                    <Link
                      className='item-page__series-link'
                      to={`/items/series/${getIdFromSeries(item.series)}`}
                    >
                      {item.series}
                    </Link>
                  </p>
