  ...
  return (
    <ul className='rating__stars'>
      <li className='rating__stars-item'>
        <button className={`rating__star rating__star_t_${rating === 5 ? 'full' : rating >= 4.5 ? 'half' : 'empty'}`}
          onClick={() => setRating({ id, value: 5 })} disabled={isLoading} />
      </li>
      <li className='rating__stars-item'>
        <button className={`rating__star rating__star_t_${rating >= 4 ? 'full' : rating >= 3.5 ? 'half' : 'empty'}`}
          onClick={() => setRating({ id, value: 4 })} disabled={isLoading} />
      </li>
      <li className='rating__stars-item'>
        <button className={`rating__star rating__star_t_${rating >= 3 ? 'full' : rating >= 2.5 ? 'half' : 'empty'}`}
          onClick={() => setRating({ id, value: 3 })} disabled={isLoading} />
      </li>
      <li className='rating__stars-item'>
        <button className={`rating__star rating__star_t_${rating >= 2 ? 'full' : rating >= 1.5 ? 'half' : 'empty'}`}
          onClick={() => setRating({ id, value: 2 })} disabled={isLoading} />
      </li>
      <li className='rating__stars-item'>
        <button className={`rating__star rating__star_t_${rating >= 1 ? 'full' : rating >= 0.5 ? 'half' : 'empty'}`}
          onClick={() => setRating({ id, value: 1 })} disabled={isLoading} />
      </li>
    </ul >
  )
}
    ...
    <div className={`rating${newClass ? ' ' + newClass : ''}`}>
      ...
      }
      {
        ...
        <div className='rating__user'>
          ...
          {
            userRating ?
              <button type='button' className='rating__reset' onClick={() => resetRating({ id: item._id })}>Сбросить оценку</button> :
