        ...
        {areItemsInCart ? (
          <>
                  ...
                  return (
                    <li key={game._id} className='cart__item'>
                      ...
                      <div className='cart__item-prices'>
                        <p
                          className={`cart__item-price${isOnSale ? ' cart__item-price_t_saleon' : ''}`}
                        >
                          {game.price * totalAmount} руб.
                        </p>
                        {isOnSale && (
                          <p className='cart__item-price cart__item-price_t_with-sale'>
                            {game.priceWithSale * totalAmount} руб.
                          </p>
                        )}
                      </div>
                      <div className='cart__amounts'>
                        {item.orders.map((order, index) => (
                          <div className='cart__amount-controls' key={index}>
                            <p className='cart__amount-platform'>
                              {getNameFromId(PLATFORMS, order.platform)}
                            </p>
                            <AmountChanger
                              key={index}
                              item={game}
                              platformToChange={order.platform}
                              newClass='cart__amount-changer'
                            />
                          </div>
                        ))}
                      </div>
            ...
            <div className='cart__data'>
              <p className={`cart__total${isWithSale ? ' cart__total_t_saleon' : ''}`}>
                {cart.totalPrice} руб.
              </p>
              {isWithSale && (
                <p className='cart__total cart__total_t_with-sale'>
                  {cart.totalPriceWithSale} руб.
                </p>
              )}
              <Link
                to='/checkout'
                onClick={handleClose}
                className='cart__button cart__button_t_checkout'
              >
                Перейти к оформлению
              </Link>
              <button
                type='button'
                className='cart__button cart__button_t_clear'
