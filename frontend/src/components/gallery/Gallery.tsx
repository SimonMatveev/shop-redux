...
const Gallery: FC<IGalleryProps> = ({ item }) => {
  ...
  const getCurrentImageIndex = () =>
    item.images.findIndex((image) => mainImgSrc === image);
  ...
  const handleSmallArrowClickLeft = () =>
  ...
  const handleSmallArrowClickRight = () =>
    setGalleryConfig((prev) => {
      return {
        ...prev,
        index: prev.index + prev.size,
  ...
  useEffect(() => {
    ...
    if (item.images.length <= 5) {
      ...
      galleryConfig.index + galleryConfig.size >= item.images.length
    ) {
      newSize = 4;
    } else {
      newSize = 3;
  ...
  useEffect(() => {
    ...
    if (
      item.images
        .slice(galleryConfig.index, galleryConfig.index + galleryConfig.size)
        .every((image) => image !== mainImgSrc) &&
      index !== -1
    ) {
