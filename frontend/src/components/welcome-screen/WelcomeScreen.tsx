...
const WelcomeScreen: FC = () => {
    ...
    const calculatePecent = useMemo(()=>
      Math.round(Math.max(...itemsToRender.map(item => (100 - (item.priceWithSale / item.price) * 100))) / 10) * 10,[itemsToRender]);
              ...
              <h1 className='welcome__title'>Добро пожаловать!</h1>
                ...
                {!isFetching ?
                    ...
                    <div className='welcome__promo-container'>
                        <p className='welcome__text'>Скидки до {calculatePecent}% - не упустите выгоду!</p>
