...
import { ENUM_CATEGORY, ENUM_FILTER_NAMES, ENUM_LOCAL_STORAGE, ENUM_PLATFORMS, IFilters } from "../../types/types";
...
export const filtersSlice = createSlice({
  name: 'filters',
  initialState: initialStateSaved ? JSON.parse(initialStateSaved) as IFilters : initialStateDefault,
  reducers: {
    setCategory: (state, { payload }: PayloadAction<{ values: any[], field: ENUM_FILTER_NAMES }>) => {
    ...
    resetFilters: (state: IFilters) => {
      for (let key of Object.keys(state)) {
        if (state.resetable.some(item => item === key) && !(state[key] === initialStateDefault[key] || (typeof state[key] === 'object' && state[key].length === 0))) {
    ...
    setFromParams: (state, { payload: searchParams }: PayloadAction<URLSearchParams>) => {
      ...
      if (searchParams.has('category')) state.category = searchParams.getAll('category') as ENUM_CATEGORY[];
      if (searchParams.has('platforms')) state.platforms = searchParams.getAll('platforms') as ENUM_PLATFORMS[];
